...
\subsection{Finding useful Context(s) of a story}
...
\begin{algorithmic}
  ...
  \State \algorithmicensure{Neighbourhood $N$}
    ...
    \ForAll {node $n \in S$}
      \State Enqueue($Q$, ($n, 1$))
    \EndFor
    \While {Empty($Q$) is {\bf False} and $|N| \leq \theta$}
      \State $(v, w)$ = Dequeue($Q$)
      \If {$w \geq \theta$}
...
\subsection{Some example News Graphs}
...
that happened in Delhi(DGR).\footnote{In no way do we want to use this unfortunate incident or others used in the paper in any way except 
